...
class Patient:
	...
	@property
	def full_name(self):
		"""getter method for full name when not None"""
		if self._full_name is None:
			self._full_name = self.fn + " " + self.ln
	...
	@phone.setter
	def phone(self, val ):
		self._phone = pn.parse("+1 {}".format(val), 'US')
		self._phone = pn.format_number(val , pn.PhoneNumberFormat.NATIONAL)

	@property
	def cont_phone(self):
		return self._cont_phone

	@cont_phone.setter
	def phone(self, val):
		self._cont_phone = pn.parse(val , 'US')
		self._cont_phone = pn.format_number(val, pn.PhoneNumberFormat.NATIONAL)
